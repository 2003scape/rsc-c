...
void bignum_divmod(struct bn *a, struct bn *b, struct bn *c,
                   struct bn *d); /* c = a/b, d = a%b */
...
int bignum_cmp(struct bn *a,
               struct bn *b); /* Compare: returns LARGER, EQUAL or SMALLER */
...
void bignum_pow(struct bn *a, struct bn *b,
                struct bn *c); /* Calculate a^b -- e.g. 2^10 => 1024 */
void bignum_pow_mod(struct bn *a, struct bn *b, struct bn *n,
                    struct bn *res); /* Calculate a^b % n */
void bignum_isqrt(struct bn *a,
                  struct bn *b); /* Integer square root -- e.g. isqrt(5) => 2*/
void bignum_assign(struct bn *dst,
                   struct bn *src); /* Copy src into dst -- dst := src */
