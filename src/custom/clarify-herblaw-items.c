...
void modify_unidentified_herbs(void) {
    ...
    game_data.items[165].description =
        mud_strdup("It can be extracted to improve coordination, but I need a "
                   "closer look to not poison myself");
    ...
    game_data.items[435].description =
        mud_strdup("I think it's a poison cure, but I need a closer look to "
                   "not poison myself");
    ...
    game_data.items[436].description =
        mud_strdup("It can be extracted to improve strength, but I need a "
                   "closer look to not poison myself");
    ...
    game_data.items[437].description =
        mud_strdup("I think it's harralander, but I need a closer look to not "
                   "poison myself");
    ...
    game_data.items[438].description =
        mud_strdup("A floral weed, I need a closer look to process it into "
                   "something useful");
    ...
    game_data.items[439].description = mud_strdup(
        "A leafy herb, but I need a closer look to not poison myself");
    ...
    game_data.items[440].description = mud_strdup(
        "I think it's avantoe, but I need a closer look to not poison myself");

    free(game_data.items[441].name);
    game_data.items[441].name = mud_strdup("Unidentified kwuarm");
    free(game_data.items[441].description);
    game_data.items[441].description = mud_strdup(
        "It's leaking foul smelling sap, I need to process it very carefully");
    ...
    game_data.items[442].description =
        mud_strdup("I think it's cadantine, but I need a closer look to not "
                   "poison myself");
    ...
    game_data.items[443].description =
        mud_strdup("It smells like dwarven settlements, but I need a closer "
                   "look to not poison myself");
    ...
    if (game_data.item_count >= 933) {
        ...
        game_data.items[933].description = mud_strdup(
            "A very unusual herb, I need a closer look to not poison myself");
    }
}
...
void modify_unfinished_potions(void) {
    ...
    if (game_data.item_count >= 1074) {
        ...
        game_data.items[1074].name =
            mud_strdup("Unfinished jangerberry potion");
    }
}
