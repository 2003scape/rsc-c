...
void mudclient_draw_lost_connection(mudclient *mud) {
    ...
    surface_draw_string_centre(mud->surface, "Connection lost! Please wait...",
                        mud->surface->width / 2, mud->surface->height / 2 - 5,
                        FONT_BOLD_14, WHITE);

    surface_draw_string_centre(mud->surface, "Attempting to re-establish",
                        mud->surface->width / 2, mud->surface->height / 2 + 13,
                        FONT_BOLD_14, WHITE);
}
